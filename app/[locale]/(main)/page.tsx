  ...
  return (
    <div className="hero-gradient min-h-[calc(100vh-4rem)] bg-muted/30">
      <section className="mx-auto grid w-full max-w-7xl gap-10 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_420px] lg:px-8 lg:py-16">
        <div className="flex min-h-[520px] flex-col justify-center gap-7">
          <div className="inline-flex w-fit items-center gap-2 rounded-md border border-border/80 bg-card/95 px-3 py-1 text-xs font-medium text-muted-foreground shadow-sm backdrop-blur">
          ...
          <form
            ...
            className="flex w-full max-w-2xl flex-col gap-3 rounded-lg border border-border/80 bg-card/95 p-2 shadow-lg shadow-foreground/5 backdrop-blur sm:flex-row"
          >
            <div className="relative min-w-0 flex-1">
              ...
              <Input
                ...
                className="h-12 border-transparent bg-muted/70 pl-9 text-base shadow-none focus-visible:bg-background"
              />
            </div>
            <Button type="submit" size="lg" className="h-12 px-5 shadow-sm shadow-primary/30 sm:px-6">
              {t("common.search")}
          ...
          <div className="flex max-w-2xl flex-wrap items-center gap-2">
            ...
            {RETAILERS.map((name) => (
              <span
                key={name}
                className="rounded-md border border-border/80 bg-background/90 px-3 py-1 text-sm font-medium text-foreground shadow-sm"
              >
                {name}
              </span>
            ))}
        ...
        <div className="grid content-center gap-3 lg:min-h-[520px]">
          <div className="rounded-lg border border-border/80 bg-card/95 p-4 shadow-lg shadow-foreground/5 backdrop-blur">
            <div className="mb-4 flex items-center justify-between border-b border-border/80 pb-3">
              <div>
                <p className="text-sm font-semibold">Price snapshot</p>
                <p className="text-xs text-muted-foreground">Same product, cleaner decisions</p>
              </div>
              <span className="rounded-md border border-emerald-200 bg-emerald-100 px-2 py-1 text-xs font-semibold text-emerald-700">
                -18%
              </span>
            </div>
            <div className="space-y-3">
              {["Amazon", "Noon", "eBay"].map((name, index) => (
                <div
                  key={name}
                  className="flex items-center justify-between rounded-md border border-border/60 bg-muted/50 p-3"
                >
          ...
          <div className="grid grid-cols-2 gap-3">
            {features.map(({ icon: Icon, title, desc }) => (
              <div
                key={title}
                className="rounded-lg border border-border/80 bg-card/95 p-4 shadow-sm backdrop-blur"
              >
